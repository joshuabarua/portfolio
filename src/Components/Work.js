	...
	return (
		<article className="pt-8 border-b-2 border-dark-content pb-5 dark:border-light-content border-opacity-20 dark:border-opacity-20">
			<div className="flex justify-between items-center">
				<h1 className="text-content md:text-lg lg:text-xl pb-4 pr-4">{position}</h1>
				<div className="btn bg-greenbg text-green-text text-xs inline-block rounded-3xl px-3 py-1 min-w-fit mb-4">{type}</div>
			</div>
			<h3 className="text-content md:text-sm  lg:text-lg font-light pl-4">{description}</h3>
			<div className="tech grid pb-2 grid-flow-col md:grid-cols-3fr gap-y-0 gap-x-0">
				{tech ? (
					tech.map((item, idx) => {
						return <img src={item} width={30} alt="" key={idx} />;
					})
				) : (
					<></>
				)}
			...
			<div className="flex justify-between pt-2">
				<div className="flex flex-col md:flex-row">
					<div className="flex items-center pr-5">
						<svg className="dark:fill-light-heading fill-dark-heading min-w-fit" width="16" height="12" viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg">
